...
for root, dirs, files in os.walk(dir):
    for filename in files:
        if filename == 'track.db':
            ...
            cursor.execute('SELECT platform, genome, name, reads, stat_mode FROM track')
...
with open(os.path.join(dir, "tracks.sql"), "w") as f:
    ...
    for row in data:
        ...
        print(f"INSERT INTO tracks (platform, genome, name, reads, stat_mode, dir) VALUES ({values});", file=f)
